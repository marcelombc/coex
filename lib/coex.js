(function (factory) {
    ...
    function loadImage(url, callback) {
        ...
        this._image.onload = function () {
            ...
            outerloop:
            for(i = 0, length = hslColors.length; i < length; i += 1) {
                for(j = 0; j < sortedHslColors.length; j += 1) {
                    if (sortedHslColors[j] > hslColors[i]) {
                        sortedHslColors.splice(j, 0, hslColors[i]);
                        continue outerloop;
                    }
                }
                sortedHslColors.push(hslColors[i]);
            }

            //Retrieving rgb colors
            for (i = 0, length = sortedHslColors.length; i < length; i += 1) {
                sortedRgbColors[i] = sortedHslColors[i][1];
            }
