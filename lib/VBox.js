(function (factory) {
    ...
    function VBox(data) {
        ...
        this._calculateVolumeAndMinMaxAxis();
    }
    ...
    /**
     * Get the colors volume.
     */
    VBox.prototype.volume = function () {
        return this._colorsVolume;
    };

    // PROTECTED METHODS

    /**
     * Calculate the volume and the min/max values for each data RGB axis.
     */
    VBox.prototype._calculateVolumeAndMinMaxAxis = function () {
        var i,
            reds = [],
            greens = [],
            blues = [],
        ...
        for (i = 0; i < this._dataLength; i += 1) {
            ...
            // Find colors volume.
            if (lastColor) {
                ratio = (Math.abs(this._data[i][0] - lastColor[0]) + Math.abs(this._data[i][1] - lastColor[1]) + Math.abs(this._data[i][2] - lastColor[2])) / 3;
            ...
            lastColor = this._data[i];
        }

        this._colorsVolume = volume;
        this._axisMinMax = [
